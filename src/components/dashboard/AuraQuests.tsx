...
export default function AuraQuests({ quests, credits, streak, activeSkin, onSkinChange, onQuestClick, onProteinLog, onClaim }: AuraQuestsProps) {
  ...
  return (
    <div className="bg-[#111111] border border-[#1f1f1f] rounded-2xl p-6 h-full flex flex-col" role="region" aria-label="Aura Quests and Rewards">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/30">
          ...
          >
            <Flame size={16} className={streak > 0 ? 'text-orange-500' : 'text-[#4b5563]'} fill={streak > 0 ? 'currentColor' : 'none'} />
          </motion.div>
          <span className="text-white font-bold text-sm">{streak}d Streak</span>
        </div>
      </div>
        ...
        {quests.map(quest => (
          <div key={quest.id}>
            ...
            <div
              ...
              className={`p-4 rounded-xl border transition-all cursor-pointer hover:bg-white/5 hover:scale-[1.01] ${quest.completed ? 'bg-green-500/5 border-green-500/30' : 'bg-[#161616] border-[#1f1f1f]'}`}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {quest.completed ? (
                    ...
                    <Circle size={20} className="text-[#4b5563]" />
                  )}
                  <span className={`text-sm font-medium ${quest.completed ? 'text-green-500' : 'text-white'}`}>{quest.label}</span>
                  {quest.id === 'protein' && (
            ...
            {quest.id === 'protein' && showProteinInput && !quest.completed && (
              <motion.div
                ...
                className="mt-2 bg-[#0d0d0d] border border-blue-500/20 rounded-xl p-4 space-y-3"
              >
                {/* Header */}
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[#4b5563] hover:text-red-400 text-[10px] font-bold uppercase transition-colors"
                  >
                    Reset Today
                  </button>
                </div>
                ...
                <div className="text-center py-2">
                  ...
                  <span className="text-[#6b7280] text-lg font-bold">g</span>
                  <p className="text-[#4b5563] text-[10px] mt-1 uppercase tracking-widest">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {QUICK_ADDS.map(g => (
                    <button
                      ...
                      className="bg-blue-500/10 hover:bg-blue-500/25 text-blue-400 font-bold text-sm py-2 rounded-xl border border-blue-500/20 hover:border-blue-500/50 transition-all active:scale-95"
                    >
                      +{g}g
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full bg-[#111111] border border-[#1f1f1f] focus:border-blue-500/50 rounded-xl px-3 py-2.5 text-white text-sm placeholder-[#4b5563] outline-none transition-colors"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6b7280] text-xs font-bold">g</span>
                  </div>
                  <button
                  ...
                  <button
                    ...
                    className="bg-[#1f1f1f] hover:bg-red-500/20 hover:text-red-400 text-[#6b7280] font-bold px-3 rounded-xl transition-colors"
                  >
                    <Minus size={16} />
                  </button>
                </div>
            ...
            {quest.id === 'protein' && quest.completed && (
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center text-green-400 text-xs font-bold py-1"
              >
      ...
      <div className="mt-6 pt-6 border-t border-[#1f1f1f]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#6b7280] text-xs uppercase">Credits</span>
          </div>
          <button
            onClick={handleClaim}
            disabled={!allCompleted}
            aria-label="Claim daily quest bonus"
            className={`px-4 py-2 rounded-xl text-xs font-bold uppercase transition-all ${allCompleted ? 'bg-[#FDB913] text-black hover:scale-105' : 'bg-[#1f1f1f] text-[#4b5563] cursor-not-allowed'}`}
          >
            Claim Daily Bonus
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border text-xs font-bold transition-all ${activeSkin === 'blue' ? 'border-[#38B6FF] bg-[#38B6FF]/10 text-[#38B6FF]' : 'border-[#1f1f1f] bg-[#161616] text-[#6b7280]'}`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border text-xs font-bold transition-all ${activeSkin === 'gold' ? 'border-[#FDB913] bg-[#FDB913]/10 text-[#FDB913]' : 'border-[#1f1f1f] bg-[#161616] text-[#6b7280]'}`}
          >
