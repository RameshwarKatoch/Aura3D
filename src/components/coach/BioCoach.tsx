...
function renderMarkdown(text: string): string {
  return text
    ...
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/^### (.+)$/gm, '<h4 class="text-white font-bold text-sm mt-3 mb-1">$1</h4>')
    .replace(/^## (.+)$/gm, '<h3 class="text-white font-semibold text-base mt-3 mb-1">$1</h3>')
    .replace(/^# (.+)$/gm, '<h2 class="text-white font-bold text-lg mt-2 mb-1">$1</h2>')
    .replace(/^\| (.+) \|$/gm, (_: string, row: string) => {
      const cells = row.split(' | ');
      return `<div class="grid gap-1 my-0.5" style="grid-template-columns:repeat(${cells.length},1fr)">${cells.map(c => `<span class="text-xs text-[#d1d5db] bg-[#1a1a1a] rounded px-2 py-1">${c.trim()}</span>`).join('')}</div>`;
    })
    .replace(/^\|[-|: ]+\|$/gm, '')
    .replace(/^- (.+)$/gm, '<div class="flex gap-1.5 my-0.5"><span class="text-primary mt-0.5 flex-shrink-0">•</span><span class="text-[#d1d5db] text-sm">$1</span></div>')
    .replace(/^\d+\. (.+)$/gm, '<div class="flex gap-1.5 my-0.5 ml-1"><span class="text-primary flex-shrink-0 text-xs mt-0.5">›</span><span class="text-[#d1d5db] text-sm">$1</span></div>')
    .replace(/\n\n/g, '<div class="h-2"></div>')
    .replace(/\n/g, '<br/>');
}
    ...
    <>
      ...
      <button
        ...
        className={`fixed right-6 bottom-32 z-50 w-12 h-12 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${
          open ? 'bg-[#1a1a1a] border border-[#2a2a2a] rotate-90' : 'bg-primary'
        }`}
        title="AI Bio-Coach"
      >
      ...
      {/* Sidebar panel */}
      <div
        className={`fixed top-0 right-0 h-full z-[60] flex flex-col bg-[#0d0d0d] border-l border-[#1f1f1f] shadow-[−8px_0_40px_rgba(0,0,0,0.5)] transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a1a1a] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-sm leading-tight">Aura Bio-Coach</p>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-green-400" />
                <p className="text-[#6b7280] text-xs">AI-powered · Knows your metrics</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                voiceMode ? 'bg-primary/20 text-primary' : 'text-[#6b7280] hover:text-white hover:bg-[#1a1a1a]'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.length === 0 && !typing && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-8">
              ...
              <div>
                <p className="text-white font-semibold mb-1">Your AI Bio-Coach</p>
                <p className="text-[#6b7280] text-sm">Ask me anything about your training, nutrition, sleep, or recovery.</p>
              </div>
              <div className="grid grid-cols-1 gap-2 w-full mt-2">
                {quickPrompts.map(p => (
                  <button
                    ...
                    className="text-left text-sm text-[#9ca3af] bg-[#111111] border border-[#1f1f1f] rounded-xl px-3 py-2.5 hover:border-primary/40 hover:text-white transition-all flex items-center gap-2"
                  >
          ...
          {messages.map(msg => (
            ...
            >
              <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 ${
                msg.role === 'coach' ? 'bg-primary/20' : 'bg-[#2a2a2a]'
              }`}>
                {msg.role === 'coach'
              ...
              <div
                className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'bg-[#111111] border border-[#1f1f1f] text-[#d1d5db] rounded-tl-sm'
                }`}
              >
                {msg.role === 'coach' ? (
                  <div
        ...
        {/* Quick prompts row */}
        {messages.length > 0 && (
          <div className="px-4 py-2 flex gap-2 overflow-x-auto flex-shrink-0 border-t border-[#111111]">
            {quickPrompts.map(p => (
              <button
                key={p}
                onClick={() => setInput(p)}
                className="text-[10px] text-[#6b7280] bg-[#111111] border border-[#1f1f1f] rounded-full px-3 py-1.5 hover:border-primary/40 hover:text-white transition-all whitespace-nowrap flex-shrink-0"
              >
                {p}
              </button>
            ))}
        ...
        {/* Input bar */}
        <div className="px-4 py-4 border-t border-[#1a1a1a] flex-shrink-0">
          <div className="flex gap-2 items-end">
            <input
              ...
              disabled={typing}
              className="flex-1 bg-[#111111] border border-[#1f1f1f] rounded-xl px-4 py-3 text-white text-sm placeholder-[#4b5563] focus:outline-none focus:border-primary transition-colors disabled:opacity-50"
            />
            <button
              onClick={handleSend}
